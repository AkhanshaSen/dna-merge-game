...
import { initScene } from './visuals/scene3d.js';
import { bindIntroAutoAdvance, pauseIntroAutoAdvance } from './visuals/intro-carousel.js';
import {
  switchView,
  pick,
  startMerge,
...
import { game, resetSession, savePersisted, resetRoundSession } from '../core/state.js';
import { setCoachForPhase } from '../content/coach-hints.js';
import { INTRO_STEP_COUNT } from '../game/game-intro.js';
...
document.getElementById('sidebar')?.addEventListener('click', (e) => {
  ...
  if (nav?.dataset.v) switchView(nav.dataset.v);
});

document.getElementById('mobile-tabs')?.addEventListener('click', (e) => {
  const tb = e.target.closest('[data-v]');
  if (tb?.dataset.v) {
    switchView(tb.dataset.v);
    document.querySelectorAll('#mobile-tabs .tb').forEach((t) => t.classList.toggle('on', t === tb));
  }
});
...
document.getElementById('mission-band')?.addEventListener('click', (e) => {
  const el = e.target.closest('[data-action]');
  if (!el) return;
  const { action } = el.dataset;
  if (action === 'open-start-game') openStartGamePrompt();
});
